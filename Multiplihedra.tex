...
\begin{abstract}
\rouge{R\'e\'ecrire \`a la toute fin}
...
\rouge{Retaper l'introduction \`a la toute fin}

\rouge{Corriger leftmargin partout o\`u n\'ecessaire}

\rouge{D\'ecider \`a la fin ce qu'on met en Th\'eor\`eme et en Proposition}
...
\subsubsection{Operads and operadic bimodules} \Guillaume{Retirer?}
...
The objects $P_n$ can be thought of as spaces encoding arity $n$ operations, while the partial compositions $\circ_i$ define how to compose these operations together, see \cite[Chapter 5]{LodayVallette12} for details. \Guillaume{See also subsection X}
...
\end{tikzpicture}}
\caption{Bijections between 2-colored trees, 2-colored nested linear graphs, and 2-colored parenthesizations. \rouge{Corriger le dessin de gauche et les parenth\'esages inclin\'es}}
...
We write only pairs of faces $(F,G)$ such that $\dim F + \dim G = \dim P$, the others can be deduced by taking faces. \rouge{Rev\'erifier que pas de probl\`eme de parenth\'esages}
...
\cref{table:numerology} shows the number of faces of complementary dimensions, as well as the number of pairs of vertices, in the cellular image of the diagonal for the multiplihedra in low dimensions.
They are compared with diagonals induced by the same orientation vector for the Loday associahedra and the permutahedra.
\Guillaume{Code for this??}

\begin{table}[h]
\centerline{\begin{tabular}{c|c|rrrrrrr|l}
\text{Pairs} $(F,G) \in \Ima\triangle_{(P,\vec v)}$ & \text{Polytopes} & 0 & 1 & 2 & 3 & 4 & 5 & 6 & \cite{OEIS} \\
\hline
& \text{Associahedra} & 1 & 2 & 6 &  &  &  &  &   \\
...
\hline
 ...
 $\dim F=\dim G =0$ & \text{Multiplihedra} & 1 & 3 & 17 & & & & &  \\
  & \text{Permutahedra} & 1 & 3 & 17 & 149 & 1809 & 28399 & 550297 &  \OEIS{A213507} 
\end{tabular}}
\caption{Number of pairs of faces in the cellular image of the diagonal of the associahedra, multiplihedra and permutahedra of dimension $0\leq n \leq 6$, induced by any good orientation vector $\vec v$. \Guillaume{A completer!!}}
\label{table:numerology}
\end{table}

The diagonal $\triangle_n$ being a section of the projection $\pi : P\times P \to P, (x,y) \mapsto (x+y)/2$ \cite[Proposition 1.1]{LA21}, one can represent its cellular image by projecting it: for each pair of faces $(F,G) \in \Ima \triangle_n$, one draws the polytope $(F+G)/2$. 
This defines a polytopal subdivision of $\J_n$.
The polytopal subdivision of $\J_3$ looks like the subdivision in \cite[Figure 3]{LA21}, and the one of $\J_4$ is illustrated on the first page of this article.
...
\begin{remark}
Note that the formula (\ref{eq:magical-formula}) does not hold for the permutahedra either, nor does it hold for the operahedra in general, see \cite[Section 3.2]{LA21}.
\end{remark}
...
The two diagonals differ exactly by four pairs of faces, represented in~\cref{fig:four-pairs} : the first diagonal includes the pairs of~(\ref{eq:quatre-paires-inclues}), while the second diagonal includes the pairs of~(\ref{eq:quatre-paires-exclues}).
...
\end{tikzpicture}}
\caption{The four pairs of~(\ref{eq:quatre-paires-inclues}) -above, in blue- and the four pairs of~(\ref{eq:quatre-paires-exclues}) -below, in red- distinguish the two possible diagonals on $\J_4$. Arrows are drawn according to the Tamari-type order. \Guillaume{Ajouter les fleches} }
...
\includegraphics[width=0.4\linewidth]{paires-choisies.png} 
\includegraphics[width=0.38\linewidth]{paires-exclues.png}}
...
\rouge{Parler \'egalement de l'associa\`edre, puis des id\'ees sur les I,J sup\'erieurs} \Guillaume{je te laisse cela!}
...
\rouge{Apr\`es avoir tap\'e tout cela, reprendre le OneNote pour voir s'il y a des id\'ees suppl\'ementaires \`a inclure}
...
We prove in section \ref{} that this is in fact the case for \emph{any} operadic tensor product, and we discuss some perspectives regarding the possibility of endowing the category of $\Ainf$-algebras with a (homotopy) symmetric monoidal structure. \rouge{R\'e\'ecrire \`a la fin}
...
\begin{definition}
We denote by $\Aalg$ the category of $\Ainf$-algebras with their $\Ainf$-morphisms.
\end{definition}
...
An $\Ainf$-algebra is a dg-algebra whose product is associative up to homotopy. 
The maps $m_n$ for $n \geq 3$ are the higher coherent homotopies arising from the fact that $m_2$ is homotopy-associative. 
An $\Ainf$-morphism is then a morphism $f_1$ which preserves the products $m_2$ up to homotopy, together with higher coherent homotopies $f_n$ for $n \geq 2$. 
For more details, we refer to \cite[Chapter 9]{LodayVallette12}. 

\subsubsection{The operad \Ainf\ and the operadic bimodule \Minf} \label{sss:operad-ainf-operadic-bimod-minf}

An operad and an operadic bimodule in the category of dg-modules over $\mathbb{Z}$ and degree $0$ chain maps between them will respectively be called a \emph{dg-operad} and a \emph{dg-operadic bimodule}.
The \emph{endomorphism operad} $\End_A$ of a dg-module $A$ is defined by $\End_A(n) := \Hom (A^{\otimes n},A)$.
For a dg-operad $P$, a \emph{$P$-algebra structure} on $A$ is the datum of a morphism of operads $P \to \End_A$.
In other words, it is a way to interpret each operation of $P_n$ in $\Hom (A^{\otimes n},A)$, such that "abstract" compositions in $P$ coincides with "actual" compositions in $\End_A$. \Guillaume{Couper?}
\Ainf-algebras are encoded by a dg-operad, denoted \Ainf. 
...
For two dg-modules $A$ and $B$, the dg-operadic bimodule $\Hom (A ,B)$ is the $(\End_B , \End_A)$-operadic bimodule defined by $ \Hom (A ,B)_n := \Hom (A^{\otimes n},B)$.
Let $R$ be a $(P,Q)$-operadic bimodule and suppose that $A$ and $B$ are respectively endowed with a $Q$-algebra structure and a $P$-algebra structure. One can then consider the induced $(P,Q)$-operadic bimodule structure on $\Hom (A ,B)$. 
A morphism of operadic bimodules $R \rightarrow \Hom (A ,B)$ can then be understood as a way to interpret the operations of $R$ as maps $A^{\otimes n} \rightarrow B$ from the $Q$-algebra $A$ to the $P$-algebra $B$. \Guillaume{Couper?}
\Ainf-morphisms between \Ainf-algebras are encoded by a dg-operadic bimodule, denoted \Minf.
...
Composition of \Ainf -morphisms can also be formulated at the level of the operadic bimodule \Minf\ as a morphism of $(\Ainf , \Ainf)$-operadic bimodules $\Minf \rightarrow \Minf \circ_{\Ainf} \Minf$, where the notation $\circ_{\Ainf}$ denotes the \emph{relative composition product} \cite[Section 11.2.1]{LodayVallette12}.
...
The composition two \Ainf -morphisms $A \rightsquigarrow B$ and $B \rightsquigarrow C$ is defined on the operadic level by the following composition of morphisms of $(\Ainf , \Ainf)$-operadic bimodules
\[ \Minf \overset{\comp}{\longrightarrow} \Minf \circ_{\Ainf} \Minf \longrightarrow \Hom (B ,C) \circ_{\End_B} \Hom (A , B) \longrightarrow \Hom ( A , C) \ . \]
...
We can then make the same choices of cellular orientations as in \cite[I, Section 4]{Mazuir21}, illustrated in \cref{fig:left-levelwise-order} :
\begin{itemize}
...
\subsubsection{Diagonals on the operad \Ainf\ and on the operadic bimodule \Minf} \Guillaume{Couper cette sous-section?}

Given $P$ and $Q$ two dg-operads and $R$ and $R'$ two dg-operadic bimodules, recall that we have defined in \cref{def:hadamard-product} their Hadamard product $P \otimes Q$ and $R \otimes R'$. \Guillaume{Couper?}

\begin{definition}[Operadic diagonals]
\begin{enumerate}[leftmargin=*] $ $
    \item A \emph{diagonal on the operad \Ainf} is a morphism of operads $\triangle : \Ainf \rightarrow \Ainf \otimes \Ainf$. 
    \item Given a diagonal on the operad \Ainf, a \emph{diagonal on the operadic bimodule \Minf} is a morphism of operadic bimodules $\triangle : \Minf \rightarrow \Minf \otimes \Minf$. where $\Minf \otimes \Minf$ is endowed with its $(\Ainf , \Ainf)$-operadic bimodule structure induced by the diagonal on \Ainf .
\end{enumerate}
\end{definition}

Given two \Ainf -algebras $A$ and $B$, it is not straightforward to define an \Ainf -algebra structure on the tensor dg-module $A \otimes B$. 
Indeed, if we define naively the operations $m_n^{A \otimes B}$ as \[ m_n^{A \otimes B}(a_1 \otimes b_1 , \cdots , a_n \otimes b_n) := \pm m_n^A(a_1 , \cdots , a_n) \otimes m_n^B (b_1 , \cdots , b_n) \ , \]
they fail to satisfy the \Ainf -equations and do not even have the expected degree.
Diagonals on the operad \Ainf\ provide a framework to solve this problem.
...
Diagonals on the operadic bimodule \Minf\ allow one to define tensor products of \Ainf -morphisms. 
For two \Ainf -morphisms $F_1 : A_1 \rightsquigarrow B_1$ and $F_2 : A_2 \rightsquigarrow B_2$, one can then define an \Ainf -morphism $F_1 \otimes F_2 : A_1 \otimes A_2 \rightsquigarrow B_1 \otimes B_2$ by the following composition
...
We postpone shortly the proofs of these two propositions, which are incorporated in the proofs of \cref{prop:diagonal-polytopale-a-infini,prop:diagonal-polytopale-m-infini} below.

\subsubsection{The polytopal diagonals on \Ainf\ and \Minf}

We will work in this section using the nested linear graphs conventions for the operad \Ainf\ and the operadic bimodule \Minf . In these notations (\Guillaume{ces dessins sont beaux, mais sont-ils n\'ecessaires?}), the operad \Ainf\ is the quasi-free dg-operad generated in arity $n \geq 2$ by one operation of degree $n-2$ represented by the trivially nested linear graph with $n$ vertices $(\bullet \cdots \bullet )$ 
...
where $\bullet \cdots \bullet$ stands for the linear graph $\ell_n$ with $n$ vertices. 
\end{proposition}
...
The signs arise from the choices of cellular orientations on the Loday associahedra made in Section \ref{sss:forcey--loday-realize} as follows. 
...
Combined with the fact that $\dim F + \dim G = \dim \K_n$, this implies that the two bases $e_j^F, e_j^G$ together form a basis of the linear span of $\K_n$. 
...
Let $A$ and $B$ be two \Ainf -algebras. We define $A \otimes_\infty B$ to be the dg-module $A \otimes B$ endowed with the \Ainf -algebra structure defined by the diagonal \diagainf . \Guillaume{Pas sur pour cette notation du tenseur infini}
\end{definition}
...
We define $F_1 \otimes_{\infty} F_2 : A_1 \otimes_\infty A_2 \rightsquigarrow B_1 \otimes_\infty B_2$ to be the \Ainf -morphism defined by the diagonal \diagminf on \Minf .
\end{definition}

Recall from \cref{ss:about} that any pair of faces $(F,G)$ of the multiplihedra satisfying the conditions of \cref{thm:formuladiagonal} always satisfies $\tp F \leq \bm G$. It would thereby be interesting to know whether the formula
\begin{align}
\triangle \left( \purple{\bullet \cdots \bullet} \right) = 
\sum_{\substack{
    \tp(\mathcal{N}) \leq \bm(\mathcal{N'}) \\
  \ |\mono(\mathcal{N})|+|\mono(\mathcal{N}')|=n-1
}}
(-1)^{|\mathrm{Ad}(\mathcal{N})\cap \mathrm{Ad}(\mathcal{N}')|}\mathrm{sgn}(\sigma_{\mathcal{N}\mathcal{N}'})\mathcal{N} \otimes \mathcal{N}' \ , 
\label{eq:diagonal-top-bot}
\end{align}
also defines a diagonal on the operadic bimodule \Minf . In other words, to know if one can directly copy the dg-formula from the polytopal diagonal on \Ainf\ to obtain a diagonal on \Minf . We prove in the following proposition that this is not the case. \rouge{Absolument d\'etailler un peu plus pour insister sur l'importance du r\'esultat au niveau polytopal !} \Guillaume{Qu'est-ce que tu veux dire ici?}
...
\begin{proof}
We computed in \cref{ss:cellular-formula} that in arity $\leq 3$, the image $\Ima \triangle$ contains all pairs of faces $\tp F\leq \bm G$ and that in arity 4, the image $\Ima \triangle_4$ contains all pairs of faces $\tp F\leq \bm G$ except the four pairs (\ref{eq:quatre-paires-exclues}).
...
The map $\triangle : \Minf \rightarrow \Minf \otimes \Minf$ is a diagonal on \Minf\ if and only if it is compatible with the differential. In particular, it has to satisfy
...
An \Ainf -category $\cat{A}$ induces an ordinary category $H_*(\cat{A})$ in homology (in fact without identity morphisms, see the following paragraphs), while an \Ainf -functor induces an ordinary functor $H_*(f) : H_*(\cat{A}) \rightsquigarrow H_*(\cat{B})$ between ordinary categories. Hence, an \Ainf -category is a category whose composition is associative up to homotopy, while an \Ainf -functor is a \emph{homotopy} functor between such categories.
...
First, we would like to know whether given three \Ainf -algebras $A$, $B$ and $C$, the two \Ainf -algebra structures $( A \otimes_\infty B) \otimes_\infty C$ and $A \otimes_\infty ( B \otimes_\infty C)$ on the dg-module $A \otimes B \otimes C$ are the same. 
In operadic terms, this amounts to ask if the diagonal \diagainf\ on \Ainf\ is coassociative or not. 
...
This proposition implies that a diagonal on the 2-colored operad \Ainfdeux\ is never coassociative.
...
We however do not prove a counterpart of \cref{prop:pas-coassoc} in general \Guillaume{Ca veut dire quoi in general?}.
...
In other words, if two \Ainf -morphisms $F_1 : A_1 \rightsquigarrow B_1$, 
...
In the case of the diagonals \diagainf\ and \diagminf , we compute that 
...
where we chose to write the formulae with trees and not nestings, as we did not introduce the definition of a 3-colored nesting of a linear graph in this paper.
...
\begin{proof}
The proof of this theorem is a simple adaptation of the results of \cite[Section 2]{MarklShnider06} in the context of 2-colored operads, applied to the minimal model \Ainfdeux\ for the 2-colored dg-operad $As^2$ encoding dg-algebras together with morphisms of dg-algebras. \Guillaume{On a ecrit qque part que dg-algebra voulait dire dg ASSOCIATIVE algebra?}
\end{proof}
...
\Guillaume{C'est bcp de blabla, personne lira cette section, il faut mettre des environnements!}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Further applications} \label{sec:furtherdirections}

\rouge{A \'ecrire \`a la fin}

\subsection{Convolution \Ainf -algebra}

\subsubsection{Retractions and diagonals}

Recall that the operad \Ainf\ is the minimal model $\Omega As^{\text{!`}}$ of the associative operad. 
There is another cofibrant replacement given by the bar-cobar (or Boardmann-Vogt) resolution $\ombas$, which is defined as the quasi-free operad 
...
where $\mathrm{PT_n}$ is the set of planar rooted trees of arity $n$ and the degree of a tree is defined as the number of its internal edges, \cite[Section 9.3]{LodayVallette12}. \Guillaume{et la diff\'erentielle??}
There is a monomorphism of operads $\Ainf \rightarrow \ombas$. 
The problem of the construction of a morphism of operads $\ombas \rightarrow \Ainf$ is more complicated and is the subject of the following proposition. 
We will call such a morphism a \textit{retraction} of \ombas\ onto \Ainf .

\begin{proposition}\label{prop:retract}
The datum of a diagonal on the operad \Ainf\ is equivalent to the datum of a morphism of dg-operads $r : \ombas \rightarrow \Ainf$ sending $\mu$ to $\mu_2$ in arity $2$.
\end{proposition}

\begin{proof}
We apply the general theory of operadic twisting morphisms \cite[Section 6.4]{LodayVallette12}, which provides us with the following isomorphisms: 
\begin{eqnarray*}
  \Hom (\Omega As^{\text{!`}}, \Omega As^{\text{!`}}) & \cong & Tw(As^{\text{!`}}, \Omega As^{\text{!`}} \otimes \Omega As^{\text{!`}}) \\
  & \cong & Tw(B As,\Omega As^{\text{!`}}) \\
  & \cong & \Hom (\ombas, \Omega As^{\text{!`}}) \ . 
\end{eqnarray*}
The only new point is to explain the second isomorphism. 
A twisting morphism $As^{\text{!`}}\to \Omega As^{\text{!`}} \otimes \Omega As^{\text{!`}}$ is, by definition, a Maurer--Cartan element in the convolution algebra associated to the convolution dgns operad $\Hom (As^{\text{!`}}, \Omega As^{\text{!`}} \otimes \Omega As^{\text{!`}})$.
This latter one is isomorphic to the desuspension $\mathcal{S}^{-1}(\Omega As^{\text{!`}} \otimes \Omega As^{\text{!`}})$.
Since the ns cooperad $As^{\text{!`}}$ is one-dimensional in each arity, and since the arity-wise linear dual dgns cooperad of the desuspended dgns operad $\mathcal{S}^{-1}(\Omega As^{\text{!`}})$ is isomorphic to the bar construction $B As$, it is again isomorphic to the following convolution dgns operad: \[ \Hom (As^{\text{!`}}, \Omega As^{\text{!`}} \otimes \Omega As^{\text{!`}}) \cong \mathcal{S}^{-1}(\Omega As^{\text{!`}} \otimes \Omega As^{\text{!`}}) \cong \Hom (\ombas, \Omega As^{\text{!`}}) \ . \]
This isomorphism of dgns operads implies the isomorphism on the level of the Maurer--Cartan elements for their associated dg pre-Lie algebras. 
The constraint $\triangle (\mu_2) = \mu_2 \otimes \mu_2$ is equivalent to $r(\mu_2)=\mu$. 
\end{proof}

\cref{prop:retract} clarifies in particular the construction of the diagonal on the operad \Ainf\ in \cite{MarklShnider06}. 
The operad \ombas\ can indeed be seen as the cellular chains on the cubical realization of the associahedra \cite[Section 9.3.1]{LodayVallette12}. 
...
M. Markl and S. Shnider then define a retraction $\ombas \rightarrow \Ainf$ and deduce a diagonal on the operad \Ainf\ from the diagram 
\[ \Ainf \longrightarrow \ombas \longrightarrow \ombas \otimes \ombas \overset{r \otimes r}{\longrightarrow} \Ainf \otimes \Ainf \ . \]
...
\begin{remark}
  It seems likely that \cref{prop:retract} admits an interpretation in terms of Morse theory, and discrete Morse theory. \Guillaume{..., citer Anibal, Martin Frankland}, see \cite[Remark 1.6]{LA21}.
...
For $f$ and $g$ two chain maps $C \rightarrow A$ define their \emph{convolution product} by
...
\begin{definition}
Let $A$ be a dg-algebra and $C$ a dg-coalgebra. The \emph{convolution algebra of $C$ and $A$} is the dg-algebra $(\Hom (C,A) , [ \partial , \cdot ] , \star)$.
\end{definition}

A Maurer-Cartan element $\alpha$ of $\Hom (C,A)$, i.e. a map $\alpha : C \rightarrow A$ such that 
\[ [ \partial , \alpha ] + \alpha \star \alpha  = 0 \ , \]
will be called a \emph{twisting morphism}. 
...
\end{definition}
The notion of an \Ainf -morphism between \Ainf -coalgebras is defined in a similar fashion. \Guillaume{N\'ecessaire?}

\begin{proposition} 
\label{prop:convolution-ainf}
Let $C$ be an \Ainf -coalgebra and $A$ be an \Ainf -algebra. 
A diagonal $\triangle$ on the operad \Ainf\ defines an \Ainf -algebra structure on the dg-module $(\Hom (C,A) , [ \partial , \cdot ])$. 
We call this \Ainf -algebra the \emph{convolution \Ainf -algebra of $C$ and $A$} defined by the diagonal $\triangle$.
\end{proposition}

\begin{proof} 
A diagonal on the operad $\Ainf$ allows one to consider the following chain of operad morphisms: 
\[ \Ainf \to \Ainf \otimes \Ainf \to \coEnd(C) \otimes \End(A) \to \Hom(C,A) \ , \]
which gives the desired structure on $\Hom(C,A)$. 
\end{proof}
...
\begin{proof}
In analogy with \cref{prop:convolution-ainf}, one can consider the chain of morphisms of $(\Ainf, \Ainf)$-operadic bimodules 
\[ \Minf \to \Minf \otimes \Minf \to \Hom(C_2,C_1) \otimes \Hom(A_1,A_2) \to \Hom (\Hom(C_1,A_1), \Hom(C_2, A_2)) \ . \] 
\end{proof}

We point out that \cref{thm:nofunctorial} implies that the constructions of \cref{prop:convolution-ainf} and \cref{prop:convolution-minf} cannot fit into a bifunctor $\mathsf{(\Ainf -cog)^{op}} \times \Aalg \rightarrow \Aalg$.

\cref{prop:convolution-ainf} implies in particular that when $C$ is an \Ainf -coalgebra and $A$ an \Ainf -algebra, one can still define \textit{twisting cochains} $\alpha : C \rightarrow A$ as Maurer-Cartan elements in the \Ainf -algebra $\Hom (C,A)$. 
\cref{prop:convolution-minf} then implies that the \Ainf -morphism $\Hom (C_1,A_1) \rightsquigarrow \Hom (C_2,A_2)$, defined by the \Ainf -morphism $F : A_1 \rightsquigarrow A_2$ and $G : C_2 \rightsquigarrow C_1$, sends twisting cochains $C_1 \rightarrow A_1$ to twisting cochains $C_2 \rightarrow A_2$.
