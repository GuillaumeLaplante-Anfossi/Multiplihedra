...
We denote by $B(\mathcal{N})$, $P(\mathcal{N})$ and $R(\mathcal{N})$ the sets of blue, purple and red nests of $\mathcal{N}$, respectively, and we define
\[
Q(\mathcal{N}) \coloneqq 
\bigcup_{\substack{R_1,\ldots,R_k \in R(\mathcal{N})\\ k \geq 0}}
\left\{ 
  \bigcup_{B \in B(\mathcal{N})} B \cup 
  \bigcup_{P \in P(\mathcal{N})} P \cup 
  \bigcup_{i=1}^k R_i 
\right\} \ ,
\]
where the braces are the set braces, and the case $k=0$ corresponds to taking $\cup R_i = \emptyset$.
\end{definition}
We order the edges of a linear graph $\gra$ from bottom to top, as represented in \cref{fig:bijections}. 
To each blue nest $B \in B(\mathcal{N})$ in a 2-colored nesting $\mathcal{N}$ of a linear graph with $n$ vertices, we associate its \emph{characteristic vector} $\vec B\in \RR^n$ which has a $1$ in position $i$ if $i \in B$ and $0$ otherwise.
To each union of nests $Q \in Q(\mathcal{N})$, we can associate the vector $\vec Q \in \RR^n$ which has a $1$ in position $i$ for $i \in Q$, a $1$ in the last ($n$th) position, and $0$ otherwise. 
We denote by $\vec n$ the vector $(1,\ldots,1) \in \RR^n$.
