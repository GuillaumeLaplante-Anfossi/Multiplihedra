...
We are indebted to Lino Amorim and Robert Lipshitz, for explaining to us their work and for their detailed insights about possible applications of our results in symplectic topology. \rouge{+ Sushmita}
...
We also compute in \cref{table:numerology} the number of faces of complementary dimensions and the number of pairs of vertices in the cellular image of the diagonal of the multiplihedra in dimension 0 to 6.
They are compared with the numbers obtained for the diagonals induced by the same orientation vector on the Loday associahedra and the permutahedra. \Guillaume{Code for this??}
...
By analogy with the case of the permutahedra \cite[Proposition 3.17]{LA21}, we conjecture that the condition $\tp F\leq \bm G$ is equivalent to the conditions in \cref{thm:formuladiagonal} for the pairs $(I,J)$ such that $|I|=|J|=1$. \rouge{Soit le prouver soit l'enlever}
...
\begin{remark}
\rouge{Coïncide bien avec celui pour les arbres ? Si oui le dire - Pas de problème dans le cas 2-coloré ?}
...
Note that the factor $-1$ comes from the fact that $\K_n$ is defined in $\RR^{n-1}$ but has dimension $n-2$. \rouge{Relire ensemble - Je ne comprends pas cette dernière phrase}
\end{proof}
...
\begin{remark}
\rouge{Lien avec les diagonales de MS et SU + dire qu'on a ajouté les signes par rapport à l'article MTTV}
\end{remark}
...
\rouge{Reste preuve coassociativité et vérification signes}
