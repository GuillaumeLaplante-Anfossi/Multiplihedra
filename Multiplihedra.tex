...
\end{tikzpicture}}
\caption{Bijections between 2-colored trees, 2-colored nested linear graphs, and 2-colored parenthesizations. \rouge{Corriger le dessin de gauche et les parenth\'esages inclin\'es (Proposition 4.13 aussi}}
...
For $\mathcal{N}$ and $\mathcal{N}'$ two 2-colored nestings of the linear graph with $n$ vertices \Guillaume{Sans condition de dimensions!}, we have that
\begin{eqnarray*}
...
\end{eqnarray*}
\rouge{Formule \`a rev\'erifier ensemble}
\end{theorem}
...
We moreover only write pairs of faces $(F,G)$ such that $\dim F + \dim G = \dim P$. \rouge{Rev\'erifier que pas de probl\`eme de parenth\'esages}
...
\cref{table:numerology} shows the number of faces of complementary dimensions, as well as the number of pairs of vertices, in the cellular image of the diagonal for the multiplihedra induced by any good orientation vector in dimensions $0$ to $6$, and compares them with the same data for the associahedra and the permutahedra.
\Guillaume{Code for this data??}

\begin{table}[h]
\centerline{\begin{tabular}{c|c|rrrrrrr|l}
\text{Pairs} $(F,G) \in \Ima\triangle_{(P,\vec v)}$ & \text{Polytopes} & 0 & 1 & 2 & 3 & 4 & 5 & 6 &  \\
\hline
...
The diagonal $\triangle_n$ being a section of the projection $\pi : P\times P \to P, (x,y) \mapsto (x+y)/2$ \cite[Proposition 1.1]{LA21}, one can represent its cellular image by projecting it via $\pi$: for each pair of faces $(F,G) \in \Ima \triangle_n$, one draws the polytope $(F+G)/2$. 
...
%quatre paires qui SONT dans la diagonale
%$\purple{\blue{\bullet \bullet \bullet}\bullet} \times \red{\bullet\red{\purple{\bullet\bullet}\bullet}}$, $\purple{\blue{\bullet\bullet\bullet}\bullet} \times \red{\bullet\blue{\bullet\bullet}\bullet}$, $\red{\blue{\bullet\bullet\bullet}\bullet} \times \red{\bullet\purple{\bullet\bullet}\bullet}$ and $\purple{\blue{\blue{\bullet\bullet}\bullet}\bullet} \times \red{\bullet\purple{\bullet\bullet}\bullet}$.
...
\rouge{Parler \'egalement de l'associa\`edre, puis des id\'ees sur les I,J sup\'erieurs}
...
\Guillaume{mettre cette section plus loin!}
...
For a pair of nestings \Guillaume{of complementary dimensions} $(\mathcal{N}, \mathcal{N}')\in \Ima\triangle^K$, the function $\sigma_{\mathcal{N}\mathcal{N}'}: \mathrm{Ad}(\mathcal{N})\sqcup \mathrm{Ad}(\mathcal{N}') \to (1,2,\ldots,|\mathrm{Ad}(\mathcal{N})\sqcup \mathrm{Ad}(\mathcal{N}')|)$ defined on $i \in \mathrm{Ad}(\mathcal{N})$ by 
\begin{equation*}
...
For a pair of 2-colored nestings \Guillaume{of complementary dimensions} $(\mathcal{N},\mathcal{N}')\in \Ima\triangle^J$, the function $\sigma_{\mathcal{N}\mathcal{N}'}: \mathrm{Ad}(\mathcal{N})\sqcup \mathrm{Ad}(\mathcal{N}') \to (1,2,\ldots,|\mathrm{Ad}(\mathcal{N})\sqcup \mathrm{Ad}(\mathcal{N}')|)$ defined on $i \in \mathrm{Ad}(\mathcal{N})$ by 
\begin{equation*}
  \sigma_{\mathcal{N}\mathcal{N}'}(i)= 
  \begin{cases}
...
where ... \rouge{Il faut harmoniser les notations de linear graph, ensemble des nestings, etc.}. \rouge{v\'erifier la formule ensemble}
\end{proposition}
...
\end{matrix} \]
\Guillaume{On a les memes signes que MS et SU}
...
where the sum runs over the pairs $\mathcal{N},\mathcal{N}' \in \mathcal{N}^2_n$ such that $\ |\mono(\mathcal{N})|+|\mono(\mathcal{N}')|=n$ and which satisfy the conditions in \cref{thm:formuladiagonal}, and where $\mono(\mathcal{N})$ denotes the set of monochrome nests of a nesting $\mathcal{N}$. \rouge{d\'efinir cette notation dans la section 3 ? ainsi les nouvelles notations $N_n$ - r\'egler le probl\`eme des parenth\'esages en italique}
\end{proposition}
