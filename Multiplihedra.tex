...
\newcommand{\gra}{\ell}
\newcommand{\Ainf}{\mathrm{A}_\infty} %A-infinity
\newcommand{\Minf}{\mathrm{M}_\infty} %A-infinity bimodule
\newcommand{\Aalg}{\ensuremath{\mathrm{A}_\infty\text{-}\mathsf{alg}}} %category A-infinity
...
\title{The diagonal of the multiplihedra and the product of $\Ainf$-categories}
...
We are also indebted to Robert Lipshitz for his detailed insights about possible applications of our results in symplectic topology.
...
From the general theory of operads, we know that the data of a $(\mathcal{P},\mathcal{Q})$-operadic bimodule $M$ is equivalent to the data of a 2-colored operad, whose algebras consists of a $\mathcal{P}$-algebra, a $\mathcal{Q}$-algebra, and a morphism between the two.
...
It seems likely that the condition $\tp F\leq \bm G$ is equivalent to the conditions in \cref{thm:formuladiagonal} for the pairs $(I,J)$ such that $|I|=|J|=1$, as it is for the permutahedron \cite[Proposition 3.17]{LA21}. 
...
where the sum runs over the pairs $\mathcal{N},\mathcal{N}' \in \mathcal{N}_2(\gra)$ such that $\ |\text{mono}(\mathcal{N})|+|\text{mono}(\mathcal{N}')|=|E(\gra)|$ which satisfy the conditions in \cref{thm:formuladiagonal}.
...
The introduction of this new algebraic structure raises questions about the properties that it satisfies. 
For simplicity we focus on $\Ainf$-algebras, however all statements have an $\Ainf$-categorical counterpart. 
We would like to know if the category of $\Ainf$-algebras is made into a symmetric monoidal category by the tensor product defined above.

\begin{enumerate}[leftmargin=*, itemsep=0.5em]
\item We first look at the category $\Aalg$ of $\Ainf$-algebras with \emph{strict} morphisms, that is linear maps $f : A \to B$ which commute with the structure operations of $A$ and $B$ (equivalently, strict morphisms are $\Ainf$-morphisms with only one non-trivial component). 
...
\begin{proposition}[{\cite[Theorem 13]{MarklShnider06}}]
  ...
  There is no coassociative diagonal for the operad $\Ainf$. Therefore, the operad $\Ainf$ does not admit the structure of a Hopf operad.  
\end{proposition} 
...
\item Let us write $\infAalg$ for the category of $\Ainf$-algebras and $\Ainf$-morphisms. 
In this case the tensor product does not even define a bifunctor $ - \otimes - : \infAalg \times \infAalg \to \infAalg$, for the following reason:
\begin{theorem}
  There is no functorial diagonal of $\Minf$, that is there is no diagonal which is compatible with the composition of $\Ainf$-morphisms. 
\end{theorem}

\begin{proof} 
  The proof is similar to the one of \cref{prop:nocoassoc}: we try to construct inductively a diagonal and we find a contradiction already in arity $2$. 

  given $\Ainf$-morphisms $F_i : A_i \rightarrow B_i$ and $G_i : B_i \rightarrow C_i$ for $i=1,2$, the following identity is satisfied 
...
\begin{proposition}
  For any tensor product of $\Ainf$-algebras, there exist $\Ainf$-isomorphisms
...
A definition was proposed recently in the context of $\infty$-categories \cite[Definition A.5.3]{Pascaleff22}, which is far more general than the one we have considered here. 
\end{enumerate}

A first direction to solve these questions is to understand the obstruction to the coassociativity of the diagonal of the operad $\Ainf$. 

\Guillaume{travail de Poliakova: integrated A-infinity algebras}
...
\subsection{Applications to symplectic topology} \label{sec:furtherdirections}


\subsubsection{Fukaya algebras}
...
\subsubsection{Bordered Heegaard Floer homology}
...
\subsubsection{Hochschild homology of Fukaya categories}
Sheel Ganatra \Guillaume{TBC}
