...
A cellular approximation for the diagonal of the associahedra was \cor{first defined at the level of cellular chains in} \cite{SaneblidzeUmble04}. 
\cor{A second construction was given in} \cite{MarklShnider06}, \cor{and was recently shown to coincide with the first one} \cite{saneblidzeComparingDiagonalsAssociahedra2022a}. 
\cor{At the topological level, a map was given for the first time in} \cite{MTTV19} \cor{and was shown to recover the previous constructions at the cellular level}. 
\cor{Such a diagonal} yields a universal formula for the tensor product of two $\Ainf$-algebras.
...
\cor{Once again, the first construction was given at the level of chains in} \cite{SaneblidzeUmble04}. \cor{We will provide the first topological map, which as a cellular map will moreover turn out to be distinct from the one of} \cite{SaneblidzeUmble04}, \cor{see} \cref{ss:about} \cor{for more details on this point}.
...
\cor{Finally, we would like to thank the anonymous referee whose detailed comments and suggestions helped improve the exposition.}
...
A \emph{cut} of a tree is a subset of edges or vertices which contains precisely one edge or vertex in each path from a \cor{leaf} to the root.
\end{definition}
...
\cor{The set of 2-colored trees endowed with the face order $\subset$ and augmented with a global minimum element $\emptyset_n$ form a poset $(\CT{n}, \subset)$ for which 2-colored atomic trees are atoms.}
...
Loday realizations of the associahedra are all generalized permutahedra (\cor{see} \cite[Corollary 2.16 (2)]{LA21}, \cor{specialized to nested linear trees}), while Forcey--Loday realizations of the multiplihedra are not. 
...
\begin{proposition} 
The \cor{generalized permutahedron} $\K_\omega\left(\tfrac{1}{2}\right)$ given by the $\tfrac{1}{2}$-lifting of the Loday realization of weight $\omega$ of the associahedron is a realization of the multiplihedron. 
\end{proposition}
\begin{proof} 
...
\cor{For any point $z$ in a positively oriented polytope, the intersection $P \cap \rho_z P$ admits a unique vertex $\bm_{\vec v}(P \cap \rho_z P)$ which minimizes the euclidean scalar product with $\vec v$, and a unique vertex $\tp_{\vec v}(P \cap \rho_z P)$ which maximizes it. These two distinguished vertices in each intersection define a diagonal map}
\begin{align*}
\begin{array}{rlcl}
...
Its cellular image, \cor{which consists of pairs of faces $(F,G)$ of $P$}, admits a combinatorial description in terms of the fundamental hyperplane arrangement of $P$, as we will now recall.
...
\begin{definition}[Coarsening projection] 
  ...
  The \emph{coarsening projection} from $P$ to $Q$ is the \cor{map} $\theta : \mathcal{L}(P)\to\mathcal{L}(Q)$ which sends a face $F$ of $P$ to the face $\theta(F)$ of $Q$ whose normal cone $\mathcal{N}_Q(\theta(F))$ is the minimal cone with respect to inclusion which contains $\mathcal{N}_P(F)$.
\end{definition}
...
For any \cor{$p+q+r=n\geq 1$}, $k\geq 2$ and any $i_1,\ldots,i_k \geq 1$, we define the \emph{action-composition maps} by 
\[
\vcenter{\hbox{
...
\]
where $R_1,\ldots,R_k \in R(\mathcal{N})$, \cor{for any $0 \leq k \leq |R(\mathcal{N})|$}, the case $\cup R_i = \emptyset$ being allowed, and where two unions that result in the same set are identified.
\end{definition}
...
\cor{For the sake of readability, we use brackets instead of parentheses for the purple nests in this representation.}
...
\begin{subfigure}{0.4\textwidth}
    ...
    \end{subfigure}
\caption{The two distinct subdivisions of the same "diamond" in $\J_4$, respectively induced by \cor{sets of} pairs~(\ref{eq:quatre-paires-inclues}) and~(\ref{eq:quatre-paires-exclues}) \cor{from \cref{prop:pas-top-bot}}.}
\label{fig:diamonds}
\end{figure}
...
\cor{For a dg module $(A,\partial)$, we endow the graded module $\Hom(A^{\otimes n}, A)$ with the differential $[\partial,f] := \partial f - (-1)^{|f|}f\partial$.}
...
We write the first factor of $\Minf \circ_{\Ainf} \Minf$ using green for the color above the \cor{diaphragm} and red for the color below the \cor{diaphragm},
...
and its second factor using blue for the color above the \cor{diaphragm} and green for the color below the \cor{diaphragm}
...
\item If $\cat{A}$ and $\cat{B}$ are \cor{strictly} unital \Ainf -categories, the tensor \Ainf -category $\cat{A} \otimes \cat{B}$ is again \cor{strictly} unital, with identity morphisms $e_{X \times Y} := e_X \otimes e_Y$ for  $X \in \mathrm{Ob}(\cat{A})$ and $Y \in \mathrm{Ob}(\cat{B})$.
\end{enumerate}
\end{proposition}
...
\cor{The goal of this section is to show that the category of \Ainf -algebras is symmetric monoidal only up to homotopy.}
