...
\Guillaume{One could also think of applications in other areas where the multiplihedron appears, such as higher category theory \cite{ForceyQuotient08}. - Compléter avec les nouvelles remarques (Robert-Nicoud--Wiejstra et Morse)}
...
They are compared with the numbers obtained for the diagonals induced by the same orientation vector on the Loday associahedra and the permutahedra. \Guillaume{Were not on OEIS before!}
...
\end{tabular}}
\caption{Number of pairs of faces in the cellular image of the diagonal of the associahedra, multiplihedra and permutahedra of dimension $0\leq n \leq 6$, induced by any good orientation vector $\vec v$.}
...
Let $t$ be a (2-colored) tree $t$. The \emph{left-levelwise order} on the vertices of $t$ is defined by ordering them from bottom to top and from left to right, proceeding one level at a time. \Guillaume{Expliquer ou juste donner référence}
\end{definition}
...
\Thibaut{Reste preuve coassociativité et vérification signes}
...
\begin{corollary}
  ...
  For any diagonal on $\Ainf$, and for any diagonal on $\Minf$, the convolution $\Ainf$-algebra $\Hom(C,A)$ does not define a bifunctor $\mathsf{(\Ainf -cog)^{op}} \times \Aalg \rightarrow \Aalg$.
\end{corollary}
\begin{proof}
...
\begin{eqnarray}
  \Hom(-,\id) &:& \mathsf{(\infAcog)^{op}} \times \Aalg \to \Aalg \label{eq:bif1} \\
  \Hom(\id, -) &:& \mathsf{(\Acog)^{op}} \times \infAalg \to \Aalg \label{eq:bif2}
\end{eqnarray}
